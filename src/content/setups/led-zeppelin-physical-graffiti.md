---
title: Led Zeppelin (Physical Graffiti)
artist: Led Zeppelin
targetTone: Épico e Espacial (Kashmir, The Rover)
guitar: Guitarra
pedals:
  - "Pure Sky"
  - "OD-3"
  - "Tank-G"
  - "Joyo EQ"
  - "Mini Universe"
  - "Lost Tempo"
amp: "MR4"
tabs:
- label: '📖 Tablatura: The Rover (Songsterr)'
  url: https://www.songsterr.com/a/wsa/led-zeppelin-the-rover-tab-s23764
- label: '📖 Tablatura: Kashmir - DADGAD (Songsterr)'
  url: https://www.songsterr.com/a/wsa/led-zeppelin-kashmir-tab-s288
- label: '📖 Tablatura: Kashmir - EBGDAE (Songsterr)'
  url: https://www.songsterr.com/a/wsa/led-zeppelin-kashmir-tab-s566091
equipment:
- title: Guitarra Cort
  list:
  - <strong class="text-white">Captador:</strong> Meio (Kashmir) ou Ponte
  - <strong class="text-white">Volume:</strong> 10  <strong class="text-white">Tone:</strong>
    8
- title: Caline Pure Sky
  tagUso: Sempre Ligado (Empurrão Médio)
  list:
  - <strong class="text-white">Volume:</strong> 5  <strong class="text-white">Gain:</strong>
    3
  - <strong class="text-white">Treble:</strong> 6  <strong class="text-white">Bass:</strong>
    4
- title: Boss OD-3
  tagUso: Saturação da Base
  list:
  - <strong class="text-white">Drive:</strong> 5  <strong class="text-white">Tone:</strong>
    6  <strong class="text-white">Level:</strong> 6
- title: M-Vave Tank-G
  list:
  - <strong class="text-white">AMP:</strong> Slot 5 (Plexi 100)
  - <strong class="text-white">Gain:</strong> 5  <strong class="text-white">Treble:</strong>
    6  <strong class="text-white">Middle:</strong> 7  <strong class="text-white">Bass:</strong>
    5
  - <strong class="text-white">CAB:</strong> Slot 7 (Marshall 1960)
  - <strong class="text-white">Modulação:</strong> PHASER (Velocidade muito lenta
    - 9h)
- title: Joyo R12 EQ (10-Band)
  tagUso: Ressonância para Afinação Alternativa
  table:
  - f: '31.2'
    db: '-2'
  - f: '62.5'
    db: '-2'
  - f: '125'
    db: '+3'
  - f: '250'
    db: '+1'
  - f: '500'
    db: '0'
  - f: 1K
    db: '+3'
  - f: 2K
    db: '+2'
  - f: 4K
    db: '0'
  - f: 8K
    db: '0'
  - f: 16K
    db: '-3'
- title: Ambiência & Loop
  list:
  - <strong class="text-white">Mini Universe:</strong> HALL ou PLATE | Mix 4 | Decay
    5 | Param 5
  - <strong class="text-white">Lost Tempo:</strong> d21 (Rock) a 82 BPM
---
