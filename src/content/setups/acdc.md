---
title: AC/DC
targetTone: Punch e Crunch Seco (Back in Black, Highway to Hell)
signalChain: 🎸 Guitarra -> Pure Sky -> OD-3 -> Tank-G -> Joyo EQ -> Mini Universe
  -> Lost Tempo -> MR4 🔊
tabs:
- label: '📖 Tablatura: Back in Black (Songsterr)'
  url: https://www.songsterr.com/a/wsa/ac-dc-back-in-black-tab-s1024
- label: '📖 Tablatura: It''s A Long Way To The Top (If You Wanna Rock ''N Roll) (Songsterr)'
  url: https://www.songsterr.com/a/wsa/ac-dc-its-a-long-way-to-the-top-if-you-wanna-rock-n-roll-tab-s8699
- label: '📖 Tablatura: It''s A Long Way To The Top (If You Wanna Rock ''N Roll) (Cifra
    Club)'
  url: https://www.cifraclub.com.br/ac-dc/its-long-way-to-the-top/
- label: '📖 Tablatura: TNT (Songsterr)'
  url: https://www.songsterr.com/a/wsa/ac-dc-tnt-tab-s2862462
equipment:
- title: Guitarra Cort
  list:
  - <strong class="text-white">Captador:</strong> Ponte (Obrigatório)
  - <strong class="text-white">Volume:</strong> 10  <strong class="text-white">Tone:</strong>
    10
- title: Caline Pure Sky
  tagUso: Sempre Ligado (Ataque da Palheta)
  list:
  - <strong class="text-white">Volume:</strong> 6  <strong class="text-white">Gain:</strong>
    1.5
  - <strong class="text-white">Treble:</strong> 7  <strong class="text-white">Bass:</strong>
    4
- title: Boss OD-3
  tagUso: Sempre Ligado (Crunch)
  list:
  - <strong class="text-white">Drive:</strong> 4 (Não exagere no ganho!)
  - <strong class="text-white">Tone:</strong> 6  <strong class="text-white">Level:</strong>
    6
- title: M-Vave Tank-G
  list:
  - <strong class="text-white">AMP:</strong> Slot 5 (Plexi 100)
  - <strong class="text-white">Gain:</strong> 4.5  <strong class="text-white">Treble:</strong>
    6  <strong class="text-white">Middle:</strong> 7  <strong class="text-white">Bass:</strong>
    4
  - <strong class="text-white">CAB:</strong> Slot 7 (Marshall 1960)
  - <strong class="text-white">Efeitos:</strong> Todos Desligados
- title: Joyo R12 EQ (10-Band)
  tagUso: 'Graves secos e médios explosivos'
  table:
  - f: '31.2'
    db: '-5'
  - f: '62.5'
    db: '-4'
  - f: '125'
    db: '-2'
  - f: '250'
    db: '0'
  - f: '500'
    db: '+1'
  - f: 1K
    db: '+2'
  - f: 2K
    db: '+4'
  - f: 4K
    db: '+3'
  - f: 8K
    db: '+1'
  - f: 16K
    db: '-3'
- title: Ambiência & Loop
  list:
  - <strong class="text-white">Mini Universe:</strong> ROOM | Mix 2 (Quase seco) |
    Decay 2 | Param 5
  - <strong class="text-white">Lost Tempo:</strong> d22 (Rock) a 92 BPM
---
